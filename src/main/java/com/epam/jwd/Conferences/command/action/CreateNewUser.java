...
public class CreateNewUser implements Command {
    ...
    private static final Logger logger = ApplicationConstants.LOGGER_FOR_CREATE_NEW_USER; //LogManager.getLogger(CreateNewUser.class);
...
//    public static final String ADMIN_CONSTANT = "ADMIN";
    ...
    public CommandResponse execute(CommandRequest request) {
        final String nickname = request.getParameter(ApplicationConstants.NICKNAME_PARAMETER_NAME);
        final String password = request.getParameter(ApplicationConstants.PASSWORD_PARAMETER_NAME);
        final String passwordRepeat = request.getParameter(ApplicationConstants.PASSWORD_REPEAT_PARAMETER_NAME);
        ...
        if (nickname == null || nickname.trim().equals("")) {
            return prepareErrorPage(request, ApplicationConstants.INVALID_NICKNAME_IS_EMPTY_MSG);
        } else if (password == null || password.trim().equals("")) {
            return prepareErrorPage(request, ApplicationConstants.INVALID_PASSWORD_IS_EMPTY_MSG);
        } else if (!validator.isLengthValid(password, ApplicationConstants.MAX_LENGTH_OF_PASSWORD)) {
            return prepareErrorPage(request, ApplicationConstants.INVALID_PASSWORD_TOO_LONG_MSG);
        } else if (!validator.isStringValid(nickname)) {
            return prepareErrorPage(request, ApplicationConstants.INVALID_NICKNAME_NOT_UTF8_MSG);
        } else if (!validator.isStringValid(password)) {
            return prepareErrorPage(request, ApplicationConstants.INVALID_PASSWORD_NOT_UTF8_MSG);
        }

        if (creatorRole == null) {
            return prepareErrorPage(request, ApplicationConstants.NO_PERMISSION_TO_CREATE_USER_IN_SYSTEM_MSG);
        }

        if (!validator.isLengthValid(nickname, ApplicationConstants.MAX_LENGTH_OF_NICKNAME_IN_DB)) {
        ...
        for (User user : users
            ...
            if (user.getNickname().equals(nickname) && !password.equals(passwordRepeat)) {
                return prepareErrorPage(request, ApplicationConstants.INVALID_NICKNAME_IS_ALREADY_EXIST_IN_SYSTEM_AND_PASSWORD_REPEATED_WRONG_MSG);
            } else if (user.getNickname().equals(nickname)) {
                return prepareErrorPage(request, ApplicationConstants.INVALID_NICKNAME_SUCH_USER_EXISTS_IN_SYSTEM_MSG);
            } else if (!password.equals(passwordRepeat)) {
                return prepareErrorPage(request, ApplicationConstants.INVALID_PASSWORD_REPEATED_WRONG_MSG);
            }
        }
        ...
        try {
            ...
            if (creatorRole.equals(ApplicationConstants.ADMIN_CONSTANT)) {
                return ApplicationConstants.USER_CREATION_SUCCESS_RESPONSE_ADMIN;
            } else {
                return ApplicationConstants.USER_CREATION_SUCCESS_RESPONSE_UNAUTHORISED;
            }

        } catch (DuplicateException e) {
            logger.info(ApplicationConstants.DUPLICATE_USER_MESSAGE);
            return prepareErrorPage(request, ApplicationConstants.DUPLICATE_USER_MESSAGE);
        }
    }
    ...
    private CommandResponse prepareErrorPage(CommandRequest request, String errorMessage) {
        ...
        return ApplicationConstants.CREATE_NEW_USER_ERROR_RESPONSE;
    }
}