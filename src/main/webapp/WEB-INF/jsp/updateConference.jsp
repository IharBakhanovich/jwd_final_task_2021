        ...
        <c:otherwise>
            <c:forEach var="conference" items="${requestScope.conferences}">
                <c:if test="${conference.id == requestScope.conferenceId}">
                    <h3><fmt:message key="label.toUpdateConferenceEditTitleChooseManagerAndPressButtonPart1"/>
                        '${conference.conferenceTitle}' <fmt:message
                                key="label.toUpdateConferenceEditTitleChooseManagerAndPressButtonPart2"/></h3>
                </c:if>
            </c:forEach>
        </c:otherwise>
    </c:choose>
        ...
        <c:when test="${not empty requestScope.error}">
            <c:choose>
                <c:when test="${sessionScope.userRole eq Role.ADMIN}">
                    ...
                    <a href="${pageContext.request.contextPath}/controller?command=show_update_conference&conferenceId=${requestScope.conferenceId}&creatorId=${sessionScope.userId}&creatorRole=${sessionScope.userRole}"><fmt:message
        ...
        <c:otherwise>
            <form action="${pageContext.request.contextPath}/controller?command=update_conference&creatorId=${sessionScope.userId}&creatorRole=${sessionScope.userRole}&conferenceId=${requestScope.conferenceId}"
                ...
                <input type="submit" value=
                    <fmt:message key="label.submitConferenceUdateButton"/> class="button">
                    <%--                            <c:choose>--%>
                    <%--                                <c:when test="${sessionScope.userRole eq Role.ADMIN}">--%>
