    ...
    <c:if test="${not empty requestScope.user}">
        ...
        <c:choose>
            <c:when test="${not empty requestScope.error}">
                ...
                <a href="${pageContext.request.contextPath}/controller?command=show_user&id=${requestScope.user.get().id}"><fmt:message
                        key="label.try_again"/></a>
            </c:when>
            <c:otherwise>
                <form action="${pageContext.request.contextPath}/controller?command=update_user&updaterId=${sessionScope.userId}&updaterRole=${sessionScope.userRole}"
                      method="post">
                    ...
                    <input type="text" id="firstNameField" name="firstName"
                           value="${requestScope.user.get().firstName}">
                    <br>
                        ...
                        <c:when test="${sessionScope.userRole eq Role.ADMIN}">
                            <c:choose>
                                <c:when test="${requestScope.user.get().role eq Role.ADMIN}">
                                    <input type="text" id="roleField" name="role"
                                           value="${requestScope.user.get().role}" readonly>
                                </c:when>
                                <c:otherwise>
                    ...
                    <c:if test="${sessionScope.userRole eq Role.ADMIN or requestScope.user.get().id == sessionScope.userId}">
                        <input type="submit" value=
                            <fmt:message key="label.updateUserDetailsButton"/> class="button">
                    </c:if>
                </form>
            </c:otherwise>
        </c:choose>
